...
namespace Metapsi
{
    public static class CsLinq
    {
        public static string Convert(Expression expression)
        {
            var jsLinqVisitor = new CsLinqVisitor();
            var jsExpression = jsLinqVisitor.Visit(expression);

            string methodBody = jsExpression.ToString();
            ...
            foreach (var constructor in jsLinqVisitor.Constructors)
            {
                methodBody = methodBody.Replace($"new {constructor.Type.Name}()", $"new {CSharp.CSharpBuilder.QualifiedTypeName(constructor.Type)}()");
            ...
            methodBody = CSharp.CSharpBuilder.ReplaceReservedKeywords(methodBody);
            ...
            methodBody = methodBody.Replace(".class", ".@class");
    ...
    public class CsLinqVisitor : ExpressionVisitor
    {
        ...
        public HashSet<NewExpression> Constructors { get; set; } = new();
        ...
        {
            Constructors.Add(node.NewExpression);
